...
class Globals {
  ...
  static async storeAndStartCronJob (...args) {
    ...
    const [urlId, expirationDate] = args
    const cronJobObjToSave = {
      data: { urlId },
    ...
    const jobPersisted = await CronSchema.create(cronJobObjToSave)
    if (jobPersisted) {
  ...
  async storeCsvUrlData (records, currentUser) {
    return new Promise(async (resolve, reject) => {
      try {
        const urlDetailsByEmail = []
        for (let i = 0; i < records.length; i++) {
          const urlDetailsByEmailObject = {}
  ...
  // async saveUrlsInCsv(records, currentUser) {
  //   // 1. Start the Mongoose session
  //   const session = await mongoose.startSession();
  //
  //   // Track successfully created records to trigger cron jobs post-commit
  //   const createdRecordsForCron = [];
  //   const urlDetailsByEmail = [];
  //
  //   try {
  //     // 2. Execute everything inside the transaction block
  //     await session.withTransaction(async () => {
  //       for (let i = 0; i < records.length; i++) {
  //         let [originalUrl, urlName, expirationDate] = records[i];
  //
  //         if (!originalUrl || !urlName || !expirationDate) {
  //           throw { code: 'BAD_REQUEST', message: exportLib.ResponseEn.INVALID_DATA };
  //         }
  //
  //         originalUrl = originalUrl.trim();
  //         urlName = urlName.trim();
  //         expirationDate = expirationDate.trim();
  //
  //         // Pass { session } to the read query
  ...
  //
  //         if (isUrlPresent) {
  //           throw { code: 'CONFLICT', message: exportLib.ResponseEn.ORIGINAL_URL_ALREADY_PRESENT };
  //         }
  //
  //         const uniqueId = Globals.getUniqueShortId();
  //         const urlObj = {
  ...
  //         };
  //
  //         // Pass { session } inside options array for .create()
  //         const [urlRecord] = await URLSchema.create([urlObj], { session });
  //
  //         if (urlRecord) {
  //           // Push to temporary arrays instead of running external cron jobs immediately
  //           createdRecordsForCron.push({ id: urlRecord._id, expirationDate });
  //
  //           urlDetailsByEmail.push({
  //             srNo: i + 1,
  //             shortUrl: config.FileUrl + '/red/' + uniqueId,
  //             urlName: urlName
  //           });
  //         }
  //       }
  //     });
  //
  //     // ---- TRANSACTION COMMITTED SUCCESSFULLY PAST THIS POINT ----
  //
  //     // 3. Trigger cron jobs safely now that data is permanently saved
  //     for (const record of createdRecordsForCron) {
  //       await Globals.storeAndStartCronJob(record.id, record.expirationDate);
  //     }
  //
  ...
  //
  //   } catch (err) {
  //     console.error('storeCsvUrlData Error:', err);
  //     throw err; // Propagate error to the controller layer
  //   } finally {
  //     // 5. Always close the session to free up connection pool resources
