...
class Globals {
  generateToken (params) {
    return new Promise(async (resolve, reject) => {
      try {
        ...
        console.log("params");
        console.log(params);
        const auth = await AuthTokens.findOne({ adminId: params.id }).select('_id').lean()
        delete params.id
        if (_.isEmpty(auth)) {
  ...
  checkToken (token) {
    return new Promise((resolve, reject) => {
      try {
        jwt.verify(token, config.access_token_secret, { ignoreExpiration: true }, async (err, decoded) => {
          if (err) {
